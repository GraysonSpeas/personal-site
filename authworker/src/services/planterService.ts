...
const slotPrices = [0, 100, 200, 1000, 10000, 20000, 40000, 60000];
const upgradePrices = [1000, 10000, 20000, 50000];
...
export async function getActivePlantersService(c: Context<{ Bindings: any }>) {
  try {
    ...
    const slotData = slots.map(s => {
      ...
      let timeRemaining = 0;
      if (seed) {
        const readyAt = new Date(String(seed.ready_at)).getTime();
        const now = Date.now();
        timeRemaining = Math.max(Math.floor((readyAt - now) / 1000), 0); // seconds
      }
      return {
    ...
    return { planted: planted.results || [], slots: slotData };
  } catch (e) {
    console.error(e);
...
export async function purchasePlanterSlotService(c: Context<{ Bindings: any }>) {
  try {
    ...
    await db.prepare('INSERT INTO userPlanterSlots (user_id, slot_index, purchased_at, level) VALUES (?, ?, datetime("now"), 1)')
...
export async function upgradePlanterSlotService(c: Context<{ Bindings: any }>) {
  try {
    const db = c.env.DB as D1Database;
    const user = await getUser(c);
    if (!user) return { error: 'Unauthorized' };
    ...
    if (typeof slotIndex !== 'number') return { error: 'Invalid slotIndex' };

    const slot = await db.prepare('SELECT level FROM userPlanterSlots WHERE user_id = ? AND slot_index = ?')
      .bind(userId, slotIndex).first<{ level: number }>();
    if (!slot) return { error: 'Slot not found' };
    if (slot.level >= MAX_UPGRADE_LEVEL) return { error: 'Max upgrade reached' };

    const price = upgradePrices[slot.level];
    if (user.gold < price) return { error: 'Not enough gold' };
...
export async function plantSeedService(c: Context<{ Bindings: any }>) {
  try {
    const db = c.env.DB as D1Database;
    const user = await getUser(c);
    if (!user) return { error: 'Unauthorized' };
    ...
    const { seedTypeId, slotIndex } = await c.req.json();
    if (typeof seedTypeId !== 'number' || typeof slotIndex !== 'number') return { error: 'Invalid input' };

    const slot = await db.prepare('SELECT level FROM userPlanterSlots WHERE user_id = ? AND slot_index = ?')
      .bind(userId, slotIndex).first<{ level: number }>();
    if (!slot) return { error: 'Slot not found' };

    const occupied = await db.prepare('SELECT id FROM plantedSeeds WHERE user_id = ? AND harvested = 0 AND slot_index = ?')
    ...
    ).bind(seedTypeId).first<{ grow_time: number, output_bait_type_id: number | null, output_resource_type_id: number | null, output_quantity: number }>();
...
export async function harvestSeedService(c: Context<{ Bindings: any }>) {
  try {
    const db = c.env.DB as D1Database;
    const user = await getUser(c);
    if (!user) return { error: 'Unauthorized' };
    ...
    const { plantedId } = await c.req.json();
    if (typeof plantedId !== 'number') return { error: 'Invalid plantedId' };
    ...
    const multiplier = dropMultipliers[Math.min(planted.level - 1, dropMultipliers.length - 1)];

    if (planted.output_bait_type_id) {
      await db.prepare(
        'INSERT INTO bait (user_id, type_id, quantity) VALUES (?, ?, ?) ON CONFLICT(user_id, type_id) DO UPDATE SET quantity = quantity + ?'
      ).bind(userId, planted.output_bait_type_id, Math.floor(planted.output_quantity * multiplier), Math.floor(planted.output_quantity * multiplier)).run();
    }
    ...
    if (planted.output_resource_type_id) {
      ...
      ).bind(userId, Math.floor(planted.output_quantity * multiplier), planted.output_resource_type_id, Math.floor(planted.output_quantity * multiplier)).run();
    }
